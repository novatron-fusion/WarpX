...
void PEC::ApplyPECtoEfield(std::array<amrex::MultiFab *, 3> Efield,
  ...
  if (patch_type == PatchType::coarse) {
    const amrex::IntVect ref_ratio =
        ((lev > 0) ? WarpX::RefRatio(lev - 1) : amrex::IntVect(1));
  ...
  // If not split E-field, the PEC is applied to the regular Efield used in
  // Maxwell's eq. If split_pml_field is true, then PEC is applied to all the
  // split field components of the tangential field.
...
#endif
  for (amrex::MFIter mfi(*Efield[0], amrex::TilingIfNotGPU()); mfi.isValid();
       ++mfi) {
    // Extract field data
    ...
    // For E-field used in Maxwell's update, nodal flag plus cells that
    // particles gather fields from in the guard-cell region are included. Note
    // that for simulations without particles or laser, ng_field_gather is 0 and
    // the guard-cell values of the E-field multifab will not be modified.
    amrex::Box const &tex =
        (split_pml_field)
            ? mfi.tilebox(Efield[0]->ixType().toIntVect())
            : mfi.tilebox(Efield[0]->ixType().toIntVect(), ng_fieldgather);
    amrex::Box const &tey =
        (split_pml_field)
            ? mfi.tilebox(Efield[1]->ixType().toIntVect())
            : mfi.tilebox(Efield[1]->ixType().toIntVect(), ng_fieldgather);
    amrex::Box const &tez =
        (split_pml_field)
            ? mfi.tilebox(Efield[2]->ixType().toIntVect())
    ...
    amrex::ParallelFor(
        ...
        [=] AMREX_GPU_DEVICE(int i, int j, int k, int n) {
          ...
          const int icomp = 0;
          PEC::SetEfieldOnPEC(icomp, domain_lo, domain_hi, iv, n, Ex, Ex_nodal,
                              fbndry_lo, fbndry_hi);
        },
        tey, nComp_y,
        [=] AMREX_GPU_DEVICE(int i, int j, int k, int n) {
          ...
          const int icomp = 1;
          PEC::SetEfieldOnPEC(icomp, domain_lo, domain_hi, iv, n, Ey, Ey_nodal,
                              fbndry_lo, fbndry_hi);
        },
        tez, nComp_z,
        [=] AMREX_GPU_DEVICE(int i, int j, int k, int n) {
          ...
          const int icomp = 2;
          PEC::SetEfieldOnPEC(icomp, domain_lo, domain_hi, iv, n, Ez, Ez_nodal,
                              fbndry_lo, fbndry_hi);
        });
  }
}
...
#pragma omp parallel if (amrex::Gpu::notInLaunchRegion())
#endif
  for (amrex::MFIter mfi(*Bfield[0], amrex::TilingIfNotGPU()); mfi.isValid();
       ++mfi) {
    ...
    // Extract tileboxes for which to loop
    // For B-field used in Maxwell's update, nodal flag plus cells that
    // particles gather fields from in the guard-cell region are included. Note
    // that for simulations without particles or laser, ng_field_gather is 0 and
    // the guard-cell values of the B-field multifab will not be modified.
    amrex::Box const &tbx =
    ...
    amrex::ParallelFor(
        ...
        [=] AMREX_GPU_DEVICE(int i, int j, int k, int n) {
          ...
          const amrex::IntVect iv(AMREX_D_DECL(i, j, k));
          const int icomp = 0;
          PEC::SetBfieldOnPEC(icomp, domain_lo, domain_hi, iv, n, Bx, Bx_nodal,
                              fbndry_lo, fbndry_hi);
        },
        tby, nComp_y,
        [=] AMREX_GPU_DEVICE(int i, int j, int k, int n) {
          ...
          const amrex::IntVect iv(AMREX_D_DECL(i, j, k));
          const int icomp = 1;
          PEC::SetBfieldOnPEC(icomp, domain_lo, domain_hi, iv, n, By, By_nodal,
                              fbndry_lo, fbndry_hi);
        },
        tbz, nComp_z,
        [=] AMREX_GPU_DEVICE(int i, int j, int k, int n) {
          ...
          const amrex::IntVect iv(AMREX_D_DECL(i, j, k));
          const int icomp = 2;
          PEC::SetBfieldOnPEC(icomp, domain_lo, domain_hi, iv, n, Bz, Bz_nodal,
                              fbndry_lo, fbndry_hi);
        });
  }
}
 ...
 **/
void PEC::ApplyPECtoRhofield(amrex::MultiFab *rho, const int lev,
  ...
  for (amrex::MFIter mfi(*rho); mfi.isValid(); ++mfi) {
    ...
    amrex::ParallelFor(mfi.validbox(), nComp,
                       [=] AMREX_GPU_DEVICE(int i, int j, int k, int n) {
                        ...
                        if (lx(iv) <= 0 || ly(iv) <= 0 || lz(iv) <= 0) {
                          rho_array(iv, n) = 0._rt;
                        }
                        });
#endif
    ...
    amrex::ParallelFor(mfi.validbox(), nComp,
          ...
          amrex::ignore_unused(j, k);
                         ...
                         PEC::SetRhoOrJfieldFromPEC(n, iv, rho_array, mirrorfac,
                                                    psign, is_pec,
...
void PEC::ApplyPECtoJfield(amrex::MultiFab *Jx, amrex::MultiFab *Jy,
  ...
  for (int idim = 0; idim < AMREX_SPACEDIM; ++idim) {
    ...
    for (int icomp = 0; icomp < 3; ++icomp) {
...
#if (defined WARPX_DIM_1D_Z)
      // For 1D : icomp=0 and icomp=1 (Ex and Ey are tangential to the z
      // boundary)
      ...
      if (is_tangent_to_bndy[icomp][idim]) {
        psign[icomp][idim][0] = (WarpX::particle_boundary_lo[idim] ==
                                 ParticleBoundaryType::Reflecting)
                                    ? 1._rt
                                    : -1._rt;
        psign[icomp][idim][1] = (WarpX::particle_boundary_hi[idim] ==
                                 ParticleBoundaryType::Reflecting)
                                    ? 1._rt
                                    : -1._rt;
      } else {
...
void PEC::ApplyPECtoElectronPressure(amrex::MultiFab *Pefield, const int lev,
                                     PatchType patch_type) {
  auto &warpx = WarpX::GetInstance();
  ...
  for (amrex::MFIter mfi(*Pefield); mfi.isValid(); ++mfi) {
    ...
    amrex::ParallelFor(mfi.validbox(), nComp,
            ...
            amrex::ignore_unused(j,k);
                         ...
                         PEC::SetNeumannOnPEC(n, iv, Pe_array, mirrorfac,
                                              is_pec, fabbox);
                       });
  }
}
