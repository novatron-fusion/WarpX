...
void FiniteDifferenceSolver::EvolveE(
    ...
    std::unique_ptr<amrex::MultiFab> const &Ffield, int lev,
    amrex::Real const dt) {
  ...
  if (m_fdtd_algo == ElectromagneticSolverAlgo::Yee) {
    ...
    EvolveECylindrical<CylindricalYeeAlgorithm>(Efield, Bfield, Jfield, Ffield,
                                                lev, dt);
#else
  if (m_grid_type == GridType::Collocated) {

    EvolveECartesian<CartesianNodalAlgorithm>(Efield, Bfield, Jfield,
                                              edge_lengths, Ffield, lev, dt);

  } else if (m_fdtd_algo == ElectromagneticSolverAlgo::Yee ||
             m_fdtd_algo == ElectromagneticSolverAlgo::ECT) {

    EvolveECartesian<CartesianYeeAlgorithm>(Efield, Bfield, Jfield,
                                            edge_lengths, Ffield, lev, dt);

  } else if (m_fdtd_algo == ElectromagneticSolverAlgo::CKC) {

    EvolveECartesian<CartesianCKCAlgorithm>(Efield, Bfield, Jfield,
                                            edge_lengths, Ffield, lev, dt);
...
#pragma omp parallel if (amrex::Gpu::notInLaunchRegion())
  ...
  for (MFIter mfi(*Efield[0], TilingIfNotGPU()); mfi.isValid(); ++mfi) {
...
#ifdef AMREX_USE_EB
          // Skip field push if this cell is fully covered by embedded
          // boundaries
          if (lx(i, j, k) <= 0) {
            Ex(i, j, k) = 0.0;
          } else {
    ...
    // Skip field push if this cell is fully covered by embedded boundaries
          ...
          // In XZ Ey is associated with a mesh node, so we need to check if the
          // mesh node is covered
          amrex::ignore_unused(ly);
          if (lx(i, j, k) <= 0 || lx(i - 1, j, k) <= 0 ||
              lz(i, j - 1, k) <= 0 || lz(i, j, k) <= 0) {
            Ey(i, j, k) = 0.0;
          } else {
#endif         
#endif
    ...
    if (Ffield) {
      ...
      amrex::ParallelFor(
          ...
          [=] AMREX_GPU_DEVICE(int i, int j, int k) {
            Ex(i, j, k) +=
                c2 * dt * T_Algo::UpwardDx(F, coefs_x, n_coefs_x, i, j, k);
          },
          [=] AMREX_GPU_DEVICE(int i, int j, int k) {
            Ey(i, j, k) +=
                c2 * dt * T_Algo::UpwardDy(F, coefs_y, n_coefs_y, i, j, k);
          },
          [=] AMREX_GPU_DEVICE(int i, int j, int k) {
            Ez(i, j, k) +=
                c2 * dt * T_Algo::UpwardDz(F, coefs_z, n_coefs_z, i, j, k);
          }
  ...
  for (MFIter mfi(*Efield[0], TilingIfNotGPU()); mfi.isValid(); ++mfi) {
    ...
    amrex::ParallelFor(
        ...
        [=] AMREX_GPU_DEVICE(int i, int j, int /*k*/) {
          Real const r =
              rmin +
              (i + 0.5_rt) *
                  dr; // r on cell-centered point (Er is cell-centered in r)
          Er(i, j, 0, 0) +=
              c2 * dt *
          ...
          for (int m = 1; m < nmodes; m++) {     // Higher-order modes
            Er(i, j, 0, 2 * m - 1) +=
                c2 * dt *
                (-T_Algo::DownwardDz(Bt, coefs_z, n_coefs_z, i, j, 0,
                                     2 * m - 1) +
                 m * Bz(i, j, 0, 2 * m) / r -
        ...
        [=] AMREX_GPU_DEVICE(int i, int j, int /*k*/) {
          ...
          if (r != 0) {                 // Off-axis, regular Maxwell equations
            ...
            for (int m = 1; m < nmodes; m++) {     // Higher-order modes
              Et(i, j, 0, 2 * m - 1) +=
                  c2 * dt *
                  (-T_Algo::DownwardDr(Bz, coefs_r, n_coefs_r, i, j, 0,
                                       2 * m - 1) +
                   T_Algo::DownwardDz(Br, coefs_z, n_coefs_z, i, j, 0,
                                      2 * m - 1) -
            ...
            for (int m = 1; m < nmodes; m++) { // Higher-order modes
              if (m == 1) {
                // The bulk equation could in principle be used here since it
                // does not diverge on axis. However, it typically gives poor
                // results e.g. for the propagation of a laser pulse (the field
                // is spuriously reduced on axis). For this reason a modified
                // on-axis condition is used here: we use the fact that
                // Etheta(r=0,m=1) should equal -iEr(r=0,m=1), for the fields Er
                // and Et to be independent of theta at r=0. Now with linear
                // interpolation: Er(r=0,m=1) = 0.5*[Er(r=dr/2,m=1) +
                // Er(r=-dr/2,m=1)] And using the rule applying for the guards
                // cells Er(r=-dr/2,m=1) = Er(r=dr/2,m=1). Thus: Et(i,j,m) =
                // -i*Er(i,j,m)
        ...
        [=] AMREX_GPU_DEVICE(int i, int j, int /*k*/) {
          ...
          if (r != 0) {                 // Off-axis, regular Maxwell equations
            Ez(i, j, 0, 0) += c2 * dt *
                              (T_Algo::DownwardDrr_over_r(
                                   Bt, r, dr, coefs_r, n_coefs_r, i, j, 0, 0) -
                               PhysConst::mu0 * jz(i, j, 0, 0)); // Mode m=0
            for (int m = 1; m < nmodes; m++) { // Higher-order modes
              Ez(i, j, 0, 2 * m - 1) +=
                  c2 * dt *
                  (-m * Br(i, j, 0, 2 * m) / r +
                   T_Algo::DownwardDrr_over_r(Bt, r, dr, coefs_r, n_coefs_r, i,
                                              j, 0, 2 * m - 1) -
    ...
    if (Ffield) {
      ...
      amrex::ParallelFor(
          ...
          [=] AMREX_GPU_DEVICE(int i, int j, int /*k*/) {
            Er(i, j, 0, 0) +=
                c2 * dt * T_Algo::UpwardDr(F, coefs_r, n_coefs_r, i, j, 0, 0);
            for (int m = 1; m < nmodes; m++) { // Higher-order modes
              Er(i, j, 0, 2 * m - 1) +=
                  c2 * dt *
                  T_Algo::UpwardDr(F, coefs_r, n_coefs_r, i, j, 0,
                                   2 * m - 1); // Real part
              Er(i, j, 0, 2 * m) +=
                  c2 * dt *
                  T_Algo::UpwardDr(F, coefs_r, n_coefs_r, i, j, 0,
          ...
          [=] AMREX_GPU_DEVICE(int i, int j, int /*k*/) {
            // Mode m=0: no update
            Real const r =
                rmin + i * dr; // r on a nodal grid (Et is nodal in r)
            if (r != 0) {      // Off-axis, regular Maxwell equations
              for (int m = 1; m < nmodes; m++) { // Higher-order modes
                Et(i, j, 0, 2 * m - 1) +=
                    c2 * dt * m * F(i, j, 0, 2 * m) / r; // Real part
                Et(i, j, 0, 2 * m) +=
                    c2 * dt * -m * F(i, j, 0, 2 * m - 1) / r; // Imaginary part
              }
            } else { // r==0: on-axis corrections
              ...
              if (nmodes >= 2) { // needs to have at least m=0 and m=1
                int const m = 1;
                Et(i, j, 0, 2 * m - 1) +=
                    c2 * dt * m * F(i + 1, j, 0, 2 * m) / dr; // Real part
                Et(i, j, 0, 2 * m) += c2 * dt * -m * F(i + 1, j, 0, 2 * m - 1) /
                                      dr; // Imaginary part
              }
            }
          },
          [=] AMREX_GPU_DEVICE(int i, int j, int /*k*/) {
            Ez(i, j, 0, 0) +=
                c2 * dt * T_Algo::UpwardDz(F, coefs_z, n_coefs_z, i, j, 0, 0);
            for (int m = 1; m < nmodes; m++) { // Higher-order modes
              Ez(i, j, 0, 2 * m - 1) +=
                  c2 * dt *
                  T_Algo::UpwardDz(F, coefs_z, n_coefs_z, i, j, 0,
                                   2 * m - 1); // Real part
              Ez(i, j, 0, 2 * m) +=
                  c2 * dt *
                  T_Algo::UpwardDz(F, coefs_z, n_coefs_z, i, j, 0,
