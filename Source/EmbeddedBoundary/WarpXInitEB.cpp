...
#ifdef AMREX_USE_EB
namespace {
class ParserIF : public amrex::GPUable {
public:
  ParserIF(const amrex::ParserExecutor<3> &a_parser) : m_parser(a_parser) {}
  ...
  operator()(AMREX_D_DECL(amrex::Real x, amrex::Real y,
...
private:
  amrex::ParserExecutor<3>
      m_parser; //! function parser with three arguments (x,y,z)
};
} // namespace
#endif
...
void WarpX::InitEB() {
  ...
  if (!impf.empty()) {
    ...
    // to which amrex should try to coarsen the EB.  It will stop after
    // coarsening as much as it can, if it cannot coarsen to that level.  Here
    // we use a big number (e.g., maxLevel()+20) for multigrid solvers.  Because
    // the coarse level has only 1/8 of the cells on the fine level, the memory
        ...
        amrex::ParallelFor(box, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
          if (edge_cent(i, j, k) == amrex::Real(-1.0)) {
            ...
            edge_lengths_dim(i, j, k) =
                1 - amrex::Math::abs(amrex::Real(2.0) * edge_cent(i, j, k));
          }
        });
      }
    }
...
void WarpX::ComputeFaceAreas(
  ...
  for (amrex::MFIter mfi(flags); mfi.isValid(); ++mfi) {
#ifdef WARPX_DIM_XZ
    // In 2D we change the extrema of the for loop so that we only have the case
    // idim=1
...
void WarpX::ScaleEdges(
  ...
  for (amrex::MFIter mfi(*edge_lengths[0]); mfi.isValid(); ++mfi) {
    ...
    for (int idim = 0; idim < AMREX_SPACEDIM; ++idim) {
#else
    WARPX_ABORT_WITH_MESSAGE("ScaleEdges: Only implemented in 2D3V and 3D3V");
#endif
      const amrex::Box &box =
          mfi.tilebox(edge_lengths[idim]->ixType().toIntVect(),
                      edge_lengths[idim]->nGrowVect());
      auto const &edge_lengths_dim = edge_lengths[idim]->array(mfi);
      amrex::ParallelFor(box, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
...
void WarpX::ScaleAreas(
  ...
  for (amrex::MFIter mfi(*face_areas[0]); mfi.isValid(); ++mfi) {
    ...
    for (int idim = 0; idim < AMREX_SPACEDIM; ++idim) {
#else
    WARPX_ABORT_WITH_MESSAGE("ScaleAreas: Only implemented in 2D3V and 3D3V");
#endif
      const amrex::Box &box =
          mfi.tilebox(face_areas[idim]->ixType().toIntVect(),
                      face_areas[idim]->nGrowVect());
#ifdef WARPX_DIM_XZ
...
void WarpX::MarkCells() {
  ...
  for (int idim = 1; idim < AMREX_SPACEDIM; ++idim) {
#else
  WARPX_ABORT_WITH_MESSAGE("MarkCells: Only implemented in 2D3V and 3D3V");
#endif
    for (amrex::MFIter mfi(*Bfield_fp[maxLevel()][idim]); mfi.isValid();
         ++mfi) {
      // amrex::Box const &box =
      // mfi.tilebox(m_face_areas[maxLevel()][idim]->ixType().toIntVect());
      const amrex::Box &box =
          mfi.tilebox(m_face_areas[maxLevel()][idim]->ixType().toIntVect(),
                      m_face_areas[maxLevel()][idim]->nGrowVect());

      auto const &S = m_face_areas[maxLevel()][idim]->array(mfi);
      auto const &flag_info_face =
          m_flag_info_face[maxLevel()][idim]->array(mfi);
      ...
      amrex::ParallelFor(box, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
        ...
        //       Here we only take care of the first two cases. The entries
        //       corresponding to the intruded faces are going to be set in the
        //       function ComputeFaceExtensions
        //     - for every face flag_ext_face contains a:
        //          * 1 if the face needs to be extended
        //          * 0 otherwise
        //       In the function ComputeFaceExtensions, after the cells are
        //       extended, the corresponding entries in flag_ext_face are set to
        //       zero. This helps to keep track of which cells could not be
        //       extended
        ...
        // The criterion is that the face has to be interior and not already
        // unstable itself
