...
$WITH_CONDA
cmake --preset $PRESET -DWarpX_CCACHE=OFF -B $BUILD_DIR
cmake --build $BUILD_DIR --target lib_3d --target app_3d --target pyamrex_pip_wheel --target pip_wheel || df -a
...
cp /opt/rocm/lib/libhiprtc.so.6.?.* $BUILD_DIR/hip
cp /opt/rocm/lib/libhiprtc-builtins.so.6.?.* $BUILD_DIR/hip
